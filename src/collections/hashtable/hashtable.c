...
hashtable_measure_capacity(size_t size) {
  return size * 2;
}

static inline int
hashtable_realloc(hashtable_t *hashtable, size_t capacity) {
...
static inline int
hashtable_requires_realloc(hashtable_t *hashtable) {
  if (hashtable_realloc(hashtable, hashtable_measure_capacity(hashtable->size)) < 0)
    return -1;
...
hashtable_insert(hashtable_t *hashtable, void *key, size_t key_size, bool key_copy, bool key_free, void *value, size_t value_size, bool value_copy, bool value_free) {
  ...
  if (hashtable->size == hashtable->capacity && hashtable_requires_realloc(hashtable) < 0)
    return -1;
