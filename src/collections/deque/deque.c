...
deque_measure_capacity(size_t size) {
  return size * 2;
}

static inline int
deque_realloc(deque_t *deque, size_t capacity) {
...
static inline int
deque_requires_realloc(deque_t *deque) {
  if (deque_realloc(deque, deque_measure_capacity(deque->size)) < 0)
    return -1;
...
deque_pushfront(deque_t *deque, void *value, size_t size, bool copy, bool free) {
  ...
  if (deque->size == deque->capacity && deque_requires_realloc(deque) < 0)
    return -1;
